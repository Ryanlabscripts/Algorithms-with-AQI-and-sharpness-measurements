function [resGM,QQ,lhSharp,resultImageOut]=OS_SART_AQI_mk8(proj,geo,angles,lH,niter,Freewheel,sharp_line,sharp_start,sharp_RHe,varargin)
%
...
for ii=1:niter
    ...
    if ii==1 % Set-up
        ...
        resGM=res;
        ii4Max=1;
        % Minimum number of iterations which must be completed = 4, so
        % initially set n2Stop to 6 (max value plus 2 more iterations
        % before STOP)
        n2Stop=6; 
        ...
        tOld=tTaken;
    elseif ii > 3 % Now check for current maximum of AQI and store x if necessary
        if QQ(ii) > QQmax
            QQmax=QQ(ii);
            resGM=res;
            ii4Max=ii;
            ...
            new_n2Stop=ii4Max+2; % Here assumed 2 more iterations before STOP
