function [resGM,QQ,lhSharp,resultImageOut]=SIRT_AQI_mk8(proj,geo,angles,lH,niter,Freewheel,sharp_line,sharp_start,sharp_RHe,varargin)
%
...
itFreewheel=false;
%
% Sharpness assessment added
% Input variables added for:
%       ID of image to be used in quality assessment (result_image)
%       Sharpness calculation parameters
%       (sharp_line,sharp_RHe)

% resGM is the reconstruction result at the iteration having the greatest AQI
% value.
...
for ii=1:niter
    ...
    if ii==1
        ...
        resultImageOut=zeros(ip,iq,niter); % Base size of (resultImageOut) on size of (res)
        clear ip iq ir;
    ...
    %     ===============================================================
    %       Investigate 'stopping' based on AQI
    %
    ...
    if ii==1 % Set-up
        ...
        resGM=res;
        ii4Max=1;
        % Minimum number of iterations which must be completed = 100, so
        % initially set n2Stop to 199 (max value plus 99 more iterations
        % before STOP)
        n2Stop=199; 
        disp(['Initial STOP iteration no. set to: ',num2str(n2Stop)]);
        if itFreewheel
        ...
        tOld=tTaken;
    elseif ii > 99 % Now check for current maximum of AQI and store x if necessary
        if QQ(ii) > QQmax
            QQmax=QQ(ii);
            resGM=res;
            ii4Max=ii;
            disp(['Maximum AQI value currently taken to be at iteration: ',num2str(ii4Max)]);
            % Now calculate a new n2Stop as an AQI maximum detected.
            new_n2Stop=ii4Max+99; % Here assumed 99 more iterations before STOP
            % N.B. Max value of new_n2Stop is 'niter', max number of iterations requested.
            new_n2Stop=min(new_n2Stop,niter);
            % Check if n2Stop must be reset. 
            if new_n2Stop > n2Stop
                n2Stop=new_n2Stop;
    ...
    % Check whether to stop iteration process
    if (itFreewheel==0 && ii==n2Stop) || (itFreewheel==1 && ii==niter)
        if (itFreewheel==0 && ii==n2Stop)
            fprintf('\n\nQuality achieved. Stopping further iterations. \n');
        else
            fprintf('\n\nFull set of iterations completed. \n');
        end
        disp(' ');
        disp(['Maximum AQI value achieved at iteration: ',num2str(ii4Max)]);
        ...
        % Create table
        T = table((1:niter)',tIterationResults,ii4MaxResults,n2StopResults,itStop,...
            lhSharp,QQ,'VariableNames',...
            {'Iteration','Iteration time','ii4Max','n2Stop','itStop','Sharpness','AQI'});
